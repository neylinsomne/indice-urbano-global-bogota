...
Para orquestar la ingesta y el procesamiento de información, se diseñó un pipeline automatizado con dos ramas que comparten origen pero alimentan funciones distintas: una \textbf{rama analítica espacial} sobre PostGIS, encargada del cálculo de los indicadores y los modelos econométricos, y una \textbf{rama vectorial} sobre PgVector que sustenta el asistente conversacional. Ambas se nutren de los mismos datos crudos almacenados en MongoDB, tras una etapa común de limpieza y geocodificación. La rama vectorial transforma la información no estructurada de los inmuebles en representaciones densas (\emph{embeddings}), habilitando consultas en lenguaje natural (e.g., \emph{``apartamentos cerca a parques seguros''}) que las consultas SQL convencionales no resuelven.

El detalle técnico de la arquitectura ---diagrama del pipeline, modelo de datos completo, volumetría por tabla, contenedores y orquestación de tareas--- se documenta en el Anexo~\ref{ax:tecnico} para mantener este capítulo enfocado en la lógica metodológica del estudio.
...
\textbf{Recomendación de política}: el $\IUG$ debe reportarse siempre acompañado del residual hedónico, no aisladamente como variable independiente que compita con el estrato. La unidad operativa del sistema es el par $(\IUG, \varepsilon)$ y la salida natural es la matriz de decisión 2$\times$2. El sistema, con sus \num{47258} registros históricos de scraping, sienta además las bases para análisis longitudinales: la trayectoria temporal del precio por barrio permitiría construir señales predictivas de valorización que ningún modelo hedónico de corte transversal puede abordar. La incorporación futura de regresión espacial (Spatial Lag, GWR), la calibración empírica de los pesos del $\IUG$ y el cruce del $\IACC$ con la expansión del metro de Bogotá son líneas naturales que la arquitectura PostGIS del sistema ya soporta.
