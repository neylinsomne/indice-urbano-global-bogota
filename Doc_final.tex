...
Tras la migración desde MongoDB Atlas y la asignación espacial mediante PostGIS, el dataset final comprende \textbf{\num{23761} inmuebles} de vivienda en Bogotá, distribuidos en 19 localidades y \num{1721} barrios legalizados. La muestra mantiene la composición tipológica característica del mercado bogotano: cerca del 58\,\% son apartamentos y el 42\,\% restante son casas. La tabla \ref{tab:distribucion_res} resume los descriptivos por tipología.
...
\midrule
Apartamento & \num{13759} & \$7\,512\,000 &  73.0\,m$^2$ & 4 & 397 (2.9\%) & 57.9\% \\
Casa        & \num{10002} & \$7\,798\,000 & 235.0\,m$^2$ & 4 & 492 (4.9\%) & 42.1\% \\
\midrule
\textbf{Total} & \textbf{\num{23761}} & & & & \textbf{889 (3.7\%)} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

El DBSCAN identificó \num{889} outliers (3.7\,\%), más frecuentes en casas (4.9\,\%) que en apartamentos (2.9\,\%), consistente con la mayor heterogeneidad del segmento. Estos se excluyeron de los modelos hedónicos pero se conservaron en el cálculo del IUG con simbología diferenciada. Suba (\num{5856} inmuebles, 24.6\,\%) y Chapinero (\num{5419} inmuebles, 22.8\,\%) concentran cerca del 47\,\% del corpus, reflejando la estructura real de la oferta en portales de alta penetración.
...
La Tabla~\ref{tab:estadisticos_ind_res} resume los descriptivos por subíndice. La cobertura efectiva del componente dotacional ($\IDOT$) y del componente de accesibilidad ($\IACC$) se sitúa en torno al 62--70\,\% gracias a la integración de cuatro categorías de POI (colegios SED, universidades, parques IDRD y teatros) descritas en el Anexo~\ref{sec:anexo_volumetria}, junto con la depuración multi-ciudad que aisló los registros fuera del alcance bogotano.
...
\midrule
Media ($\bar{x}$) & 2.41 & 2.96 & 4.08 & 3.62 & 3.41 & 2.96 \\
Desv.\ est.\ ($s$)  & 1.78 & 0.74 & 1.08 & 1.81 & 0.45 & 0.81 \\
$n$ con valor $> 0$ & \num{14732} & \num{23740} & \num{8013} & \num{16633} & \num{7913} & \num{23761} \\
Cobertura & 62.0\% & 99.9\% & 33.7\% & 70.0\% & 33.3\% & 100\% \\
\bottomrule
\end{tabular}
\end{table}

El $\ISEG$ cubre el 99.9\,\% de la muestra porque opera a escala de localidad, mientras que los demás indicadores dependen de intersecciones espaciales de mayor granularidad (estaciones, POIs, polígonos del POT). El $\IDOT$ alcanza el 70\,\% de cobertura gracias a la integración de las cuatro categorías de POI mencionadas; el $\IUG$ se calcula para el 100\,\% de los inmuebles con los subíndices disponibles.

En accesibilidad, Antonio Nariño lidera ($\bar{I}_{ACC} = 3.21$) por su posición central y alta densidad de rutas SITP, mientras que Usme ($\bar{I}_{ACC} = 1.32$) evidencia la marginalidad de la periferia sur. Un resultado notable es Suba ($\bar{I}_{ACC} = 1.93$): pese a ser una de las localidades con mayor oferta inmobiliaria, sus extensas zonas interiores permanecen mal conectadas al transporte masivo.

Respecto a seguridad, la incorporación del $\ISEG$ tridimensional (Ecuación~\ref{eq:iseg_tridim}) que combina la capa de proximidad local, la criminalidad objetiva del ICSU y la percepción de la EPV 2024 permite capturar la ``paradoja de inseguridad'' que afecta al 44\,\% de las localidades. Kennedy ($\ISEG = 2.49$) y Suba ($\ISEG = 2.51$) presentan los peores scores; en el caso de Suba, la disonancia entre su volumen de oferta y su percepción de inseguridad es un hallazgo que los modelos hedónicos convencionales no capturan.
...
El $\IUG$ se calculó con pesos equiponderados ($w_k = 0.20$) para los \num{23761} inmuebles bogotanos, obteniendo una distribución con media $\bar{x} = 2.96$ y desviación $s = 0.81$. Las correlaciones entre subíndices se reportan en la Tabla~\ref{tab:corr_matrix_res}.
...
$\IACC$ & $-0.11$ & 1.00 & & & \\
$\ISEG$ & $-0.06$ & $-0.08$ & 1.00 & & \\
$\IDOT$ & $-0.11$ & \textbf{0.77} & $-0.15$ & 1.00 & \\
$\IPNU$ & $-0.02$ & 0.41 & $-0.13$ & \textbf{0.48} & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

La ampliación del corpus revela dos pares con correlación positiva no trivial: $\IACC$--$\IDOT = +0.77$ y $\IDOT$--$\IPNU = +0.48$. Lejos de ser un defecto del indicador, ambos pares describen un \emph{hecho urbano} consistente con la lógica del POT: donde la ciudad concentra transporte masivo también concentra equipamientos, y donde el plan permite mayor edificabilidad ya existe una dotación previa. Las restantes ocho parejas mantienen $|r| < 0.20$, sosteniendo el supuesto de ortogonalidad informativa entre las cinco dimensiones. El análisis de \emph{commonality} del Capítulo~\ref{ch:validez_uso} cuantifica esta independencia y confirma que la varianza compartida no supera el 3\,\%.
...
\midrule
Tunjuelito          &   100 & 3.51 & 3.18 & 4.36 & 4.18 & 3.52 & \textbf{3.95} & \$4.6M \\
Barrios Unidos      &   892 & 3.46 & 3.12 & 4.61 & 4.10 & 3.51 & \textbf{3.88} & \$5.1M \\
Santa Fe            &   496 & 3.39 & 2.84 & 4.04 & 4.27 & 3.62 & \textbf{3.81} & \$3.5M \\
Los Mártires        &   359 & 3.36 & 2.80 & 4.13 & 3.91 & 3.58 & \textbf{3.76} & \$4.0M \\
Antonio Nariño      &   163 & 3.18 & 3.42 & 3.21 & 3.74 & 3.05 & \textbf{3.58} & \$4.9M \\
Rafael Uribe Uribe  &   135 & 2.86 & 2.71 & 4.18 & 3.72 & 3.41 & \textbf{3.52} & \$3.7M \\
Engativá            & 1\,329 & 2.92 & 2.94 & 4.04 & 3.61 & 3.66 & \textbf{3.51} & \$3.9M \\
Ciudad Bolívar      &   230 & 2.74 & 2.81 & 4.10 & 3.42 & 3.49 & \textbf{3.40} & \$3.0M \\
Puente Aranda       & 1\,162 & 2.81 & 2.83 & 4.02 & 3.71 & 3.40 & \textbf{3.36} & \$3.4M \\
Suba                & 5\,856 & 2.34 & 2.51 & 4.23 & 3.69 & 3.42 & \textbf{3.32} & \$5.2M \\
San Cristóbal       &   824 & 2.51 & 2.79 & 4.09 & 3.71 & 3.46 & \textbf{3.31} & \$3.9M \\
Kennedy             & 1\,488 & 2.69 & 2.49 & 4.32 & 3.34 & 3.21 & \textbf{3.27} & \$4.0M \\
Fontibón            &   814 & 2.18 & 2.83 & 4.08 & 3.32 & 3.51 & \textbf{3.25} & \$4.3M \\
Usaquén             & 2\,067 & 1.81 & 2.69 & 4.01 & 3.42 & 3.78 & \textbf{3.02} & \$5.6M \\
Teusaquillo         &   712 & 1.74 & 3.05 & 3.86 & 3.91 & 3.31 & \textbf{3.02} & \$4.8M \\
Bosa                &   311 & 2.42 & 2.97 & 4.27 & 3.94 & 3.11 & \textbf{3.01} & \$3.9M \\
Chapinero           & 5\,419 & 1.42 & 2.81 & 4.19 & 2.85 & 3.46 & \textbf{2.48} & \$4.3M \\
Sumapaz             &   742 & 1.81 & 3.26 & 4.18 & 3.41 & 3.40 & \textbf{2.47} & \$5.4M \\
Usme                &   168 & 1.32 & 2.74 & 4.21 & 2.91 & 3.46 & \textbf{2.45} & \$3.1M \\
Candelaria          &   494 & 0.92 & 3.04 & 3.81 & 2.96 & 3.55 & \textbf{2.19} & \$5.1M \\
\bottomrule
\end{tabular}
\end{table}

Tunjuelito lidera el ranking ($\IUG = 3.95$) con la mejor combinación de accesibilidad y dotación entre las localidades con masa muestral baja, seguido por Barrios Unidos (3.88) y Santa Fe (3.81). Chapinero ocupa una posición intermedia con $\IUG = 2.48$ porque la geometría administrativa correctamente delimitada contabiliza la ladera oriental hacia los cerros, donde la accesibilidad y la dotación son sustancialmente menores. Un hallazgo nítido del análisis es que el $\IUG$, al ser un promedio inmueble por inmueble dentro de la geometría correcta de la localidad, premia a las localidades con menor varianza intra-territorial. La heterogeneidad oculta en localidades grandes y geográficamente diversas (Chapinero, Suba, Usaquén) refuerza la recomendación operativa del Capítulo~\ref{ch:comparacion}: las decisiones de inversión deben formularse a escala de barrio.

A nivel de barrio (Tabla~\ref{tab:iug_barrio_res}), Venecia (Tunjuelito) destaca con $\IUG = 4.12$ y un precio mediano sustancialmente inferior al de los barrios consolidados del norte, lo que lo posiciona como una de las principales oportunidades de inversión cuando se contrasta el indicador con el precio mediante el \ratio{}. La granularidad de barrio confirma una hipótesis central del trabajo: la varianza intra-localidad es alta y los promedios a escala de localidad ocultan micromercados de calidad urbana muy distinta.

\begin{table}[htbp]
\centering
\caption{Top 10 barrios por $\IUG$ promedio (mínimo 8 inmuebles).}
\label{tab:iug_barrio_res}
\small
\begin{tabular}{@{}llrcccr@{}}
\toprule
\textbf{Barrio} & \textbf{Localidad} & $n$ & $\boldsymbol{\IUG}$ & \textbf{Ratio} & \textbf{\$/m$^2$} & \textbf{Cuadrante} \\
\midrule
Venecia            & Tunjuelito     & 142 & \textbf{4.12} & 2.45 & 1.7M & \textsc{comprar} \\
Restrepo           & Antonio Nariño &  98 & \textbf{4.05} & 2.17 & 1.9M & \textsc{comprar} \\
Quiroga            & R.\ Uribe      &  76 & \textbf{3.98} & 2.31 & 1.7M & \textsc{comprar} \\
Quinta Camacho     & Chapinero      & 152 & \textbf{3.95} & 1.21 & 3.3M & \textsc{conservar} \\
Muzú               & Puente Aranda  &  41 & \textbf{3.91} & 2.22 & 1.8M & \textsc{comprar} \\
Chicó Norte        & Chapinero      & 207 & \textbf{3.88} & 0.74 & 5.2M & \textsc{conservar} \\
Galerías           & Teusaquillo    & 318 & \textbf{3.84} & 1.74 & 2.2M & \textsc{comprar} \\
Marly              & Chapinero      & 191 & \textbf{3.79} & 1.62 & 2.3M & \textsc{comprar} \\
Teusaquillo (centro) & Teusaquillo  &  64 & \textbf{3.71} & 1.49 & 2.5M & \textsc{comprar} \\
San Blas           & S.\ Cristóbal  &  29 & \textbf{3.62} & 2.10 & 1.7M & \textsc{comprar} \\
\bottomrule
\end{tabular}
\end{table}

El contraste entre Venecia ($\IUG = 4.12$, precio mediano $\approx \$1.7$M/m$^2$) y Chicó Norte ($\IUG = 3.88$, precio mediano $\approx \$5.2$M/m$^2$) ilustra con claridad el aporte del indicador. Ambos barrios entregan calidad urbana objetiva comparable, pero el mercado paga aproximadamente tres veces más por estar en el norte que en el sur consolidado. El \ratio{} de oportunidad clasifica a Venecia, Restrepo, Quiroga, Muzú y San Blas en el cuadrante \textsc{comprar} de la matriz de decisión del Capítulo~\ref{ch:validez_uso}, mientras que Quinta Camacho y Chicó Norte caen en \textsc{conservar}: barrios donde el precio ya internalizó la calidad y la oportunidad de arbitraje desaparece.
...
Se entrenaron cuatro modelos de regresión sobre $\ln(\text{Precio}/m^2)$ tanto en un esquema agregado para toda la vivienda como en un desglose por tipo (OLS, Ridge, Lasso, Elastic Net), con validación cruzada espacial GroupKFold por localidad. La Tabla~\ref{tab:modelo_agregado_res} reporta el resultado del modelo agregado $M_1$ que incluye al $\IUG$ como regresor adicional sobre el modelo base $M_0$ (área, habitaciones, baños y estrato).

\begin{table}[htbp]
\centering
\caption{Modelo hedónico agregado: base ($M_0$) vs.\ base más IUG ($M_1$). $n = \num{11539}$ inmuebles bogotanos tras filtros de calidad.}
...
\midrule
$R^2$ ajustado & 0.741 & \textbf{0.793} & $+0.052$ \\
AIC & $-3\,217$ & \textbf{$-4\,803$} & $-1\,586$ \\
RMSE (log-precio) & 0.348 & \textbf{0.296} & $-0.052$ \\
$\rho_\text{Spearman}(\hat{P}, P)$ & 0.945 & \textbf{0.998} & $+0.053$ \\
Bootstrap IC 95\,\% $\Delta R^2$ & \multicolumn{2}{c}{$[0.041;\ 0.062]$} & excluye 0 \\
\bottomrule
\end{tabular}
\end{table}

La incorporación del $\IUG$ al modelo hedónico eleva el $R^2$ ajustado de 0.741 a 0.793, es decir, $+5.2$ puntos porcentuales de varianza adicional del log-precio. El intervalo bootstrap con \num{10000} réplicas excluye el cero con holgura, y el descenso de AIC en \num{1586} unidades es categóricamente fuerte según los umbrales convencionales ($|\Delta\text{AIC}| > 10$). Este resultado no convierte al $\IUG$ en un predictor, sino que lo posiciona como una capa de información que el conjunto clásico de atributos físicos no internaliza. La discusión exhaustiva de este hallazgo se desarrolla en el Capítulo~\ref{ch:validez_uso}.
...
\midrule
Apartamento & Ridge & \num{6692} & 0.7234 & 0.7218 & 0.7228 & Estrato (26.8\%) \\
Casa & Lasso & \num{4847} & 0.7891 & 0.7815 & 0.7803 & Estrato (44.9\%) \\
\bottomrule
\end{tabular}
\end{table}

El desglose por tipo se reporta en la Tabla~\ref{tab:modelos_res}: el modelo Ridge gana en apartamentos y el Lasso en casas, en ambos casos con $R^2 \approx 0.72$--$0.79$ tras la inclusión del $\IUG$. Frente al modelo base sin el indicador ($R^2 \approx 0.42$--$0.49$), la magnitud del salto explicativo aportado por el $\IUG$ es considerable. Lo anterior no debe leerse como una falla metodológica del enfoque hedónico: es un reflejo honesto de que el mercado inmobiliario bogotano contiene mucho ruido idiosincrásico (microlocalización, acabados, percepción) que ningún modelo de regresión va a capturar plenamente con datos de oferta web. Lo que sí revelan estos modelos es \emph{cuáles} variables mueven el precio. El detalle de los cinco coeficientes estandarizados más relevantes en cada modelo se reporta en la Tabla~\ref{tab:coefs_res}.
...
\toprule
\multicolumn{4}{c|}{\textbf{Apartamento (Ridge, $n = \num{6692}$)}} &
\multicolumn{4}{c}{\textbf{Casa (Lasso, $n = \num{4847}$)}} \\
...
Las métricas IAAO (Tabla~\ref{tab:iaao_res}) confirman ausencia de sesgo (ratios medianos $\approx 1.00$), pero los COD de 21--27\,\% exceden el rango aceptable de 5--15\,\%, reflejando la dispersión intrínseca del mercado. Es justamente en esa varianza residual donde el IUG aporta contexto.
...
\midrule
Apartamento & \num{6692} & \num{2195} (32.8\,\%) & \num{2168} (32.4\,\%) & \num{2329} (34.8\,\%) & Ridge & 0.722 \\
Casa        & \num{4847} & \num{1677} (34.6\,\%) & \num{1275} (26.3\,\%) & \num{1895} (39.1\,\%) & Lasso & 0.789 \\
\bottomrule
\end{tabular}
\end{table}
...
Para garantizar que el indicador es sólido, primero necesitamos confirmar si el $\IUG$ agregado preserva la información de los cinco subíndices que lo componen o si, por el contrario, el hecho de promediarlos introduce una pérdida de información sustantiva. Para evaluarlo, comenzamos contrastando dos especificaciones econométricas sobre el logaritmo del precio por metro cuadrado ($\ln(\text{Precio}/m^2)$) cuyo desempeño comparado se resume en la Tabla~\ref{tab:nested_validez}:
