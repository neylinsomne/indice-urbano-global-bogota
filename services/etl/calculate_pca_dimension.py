...
def calcular_pca_tipo(df_tipo, tipo_nombre):
    """Calcula PCA para un tipo de inmueble con validacion KMO/Bartlett."""

    # Importar validador
    import sys, os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'api'))
    from services.pca_validator import validar_pca, calcular_con_fallback

    print(f"\n[PCA] ========== {tipo_nombre} ==========")
    print(f"[PCA] Muestras: {len(df_tipo)}")

    # Variables para PCA (SOLO fisicas de tamano)
    features = ['area_construida', 'area_privada', 'habitaciones', 'banos']
    X = df_tipo[features].copy()

    # Estadisticas antes de imputacion
    for col in features:
    ...
    # 2. Validacion estadistica (KMO + Bartlett)
    X_np = X_imputed.values
    validacion = validar_pca(X_np, features)
    print(validacion.resumen())

    # 3. Calcular con fallback automatico
    resultado_pca = calcular_con_fallback(X_np, features, n_components=1)
    metodo = resultado_pca['metodo']

    # Recuperar scaler para guardar mean/std
    scaler = StandardScaler()
    Z = scaler.fit_transform(X_imputed)

    # Usar pesos del metodo elegido
    pesos_dict = resultado_pca['pesos']
    loadings = np.array([pesos_dict[f] for f in features])
    explained_var = resultado_pca['varianza_explicada']

    print(f"[PCA] Metodo usado: {metodo}")
    print(f"[PCA] Varianza explicada: {explained_var:.2%}")
    print(f"[PCA] Loadings (phi) - Variables fisicas de tamano:")
